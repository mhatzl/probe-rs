...
use crate::interface::RttInterface;
use crate::rtt::{DownChannel, Rtt, ScanRegion, UpChannel};
...
impl RttActiveChannel {
    ...
    pub fn poll_rtt(&mut self, core: &mut dyn RttInterface) -> Option<usize> {
        if let Some(channel) = self.up_channel.as_mut() {
            ...
            for _loop_count in 0..10 {
                match channel.read(core, self.rtt_buffer.0.as_mut()) {
                    ...
                    Err(err) => {
                        if matches!(err, crate::rtt::Error::Interface(_)) {
    ...
    pub fn _push_rtt(&mut self, core: &mut dyn RttInterface) {
