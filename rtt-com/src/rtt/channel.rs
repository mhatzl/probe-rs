use crate::{interface::RttInterface, rtt::Error};
...
impl Channel {
    ...
    pub(crate) fn validate_core_id(&self, core: &mut dyn RttInterface) -> Result<(), Error> {
        if core.core_id() == self.core_id {
            Ok(())
        } else {
            Err(Error::IncorrectCoreSpecified(self.core_id, core.core_id()))
        }
    }
...
impl UpChannel {
    ...
    pub fn mode(&self, core: &mut dyn RttInterface) -> Result<ChannelMode, Error> {
    ...
    ///
    /// See [`ChannelMode`] for more information on what the modes mean.
    pub fn set_mode(&self, core: &mut dyn RttInterface, mode: ChannelMode) -> Result<(), Error> {
        self.0.validate_core_id(core)?;
        let flags = core.read_word_32((self.0.ptr + Channel::O_FLAGS as u32).into())?;
    ...
    pub fn read(&self, core: &mut dyn RttInterface, buf: &mut [u8]) -> Result<usize, Error> {
        self.0.validate_core_id(core)?;
    ...
    /// buffer.
    pub fn peek(&self, core: &mut dyn RttInterface, buf: &mut [u8]) -> Result<usize, Error> {
        self.0.validate_core_id(core)?;
...
impl DownChannel {
    ...
    pub fn write(&self, core: &mut dyn RttInterface, mut buf: &[u8]) -> Result<usize, Error> {
        self.0.validate_core_id(core)?;
