...
impl Rtt {
    ...
    pub fn attach(core: &mut dyn RttInterface, memory_map: &[MemoryRegion]) -> Result<Rtt, Error> {
...
pub enum Error {
    ...
    /// Wraps errors propagated up from the memory interface.
    #[error("Error communicating with the target: {0}")]
    Interface(#[from] crate::interface::MemoryError),
